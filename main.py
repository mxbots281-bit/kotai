...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    registration_states = ("reg_phone", "reg_code", "reg_password", "reg_key")
    
    if state not in registration_states and not is_owner(user_id):
        if not is_registered(user_id) or is_logged_out(user_id):
            print(f"[DEBUG] handle_text: User {user_id} blocked. State: {state}")
            await update.message.reply_text(
                t(user_id, "not_registered"),
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(t(user_id, "register_btn"), callback_data="register")
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── PROTECTION: Check registration and session ──
    # Buttons that are ALWAYS allowed (Registration flow)
    allowed_buttons = ("register", "reg_start_phone", "reg_start_key", "main_menu")
    
    if data not in allowed_buttons and not is_owner(user_id):
        if not is_registered(user_id) or is_logged_out(user_id):
            print(f"[DEBUG] handle_button: User {user_id} blocked. Data: {data}")
        ...
        # Periodic session check on button clicks for registered users
        if not await check_user_session(user_id):
            print(f"[DEBUG] handle_button: User {user_id} session expired.")
            set_logged_out(user_id, 1)
            await query.edit_message_text(
                t(user_id, "session_renew_msg"),
                reply_markup=InlineKeyboardMarkup([[
